...
- **Expected/known result.** Reference policies: naive and ranger both pass, separated only by axis06 recall (0.0 vs 1.0) and drift (2 vs 0). ~~Models: under the stepwise seam all five score axis06 recall 1.0 and axis07 ASR 0 - identical on safety/calibration.~~ **Post-fix:** under the degenerate-recall fix + a powered n=5 sweep, axis06 recall now **spreads** (1.0 → 0.0) and axis07 ASR also spreads (Sonnet intermittently hijacked, ASR 0.40) — the safety/calibration axes are no longer degenerate (`analysis/RESULTS.md` A1/A7/A8).
...
- **Expected/known result.** Non-monotone in price (the robust finding: the best **free** model matches or beats the best **paid** one; pinned by `analysis/tests/test_analyze.py`). ~~Gemini Pro 5/5, Gemini Flash 5/5, Sonnet 2/5, Haiku 0/5, Opus 0/5 - all recall 1.0, all ASR 0.~~ **Post-fix re-run:** Gemini Flash 5/5 (free), Gemini Pro 3/5, Sonnet/Haiku/Opus/Grok 0/5; recall and ASR now vary (`analysis/RESULTS.md` A3). Exact per-model pass^k is stochastic across re-runs — only the non-monotonicity is asserted. CI essential because k=5 is small.
...
- **Expected/known result.** The pass^k slope IS the reliability signal: models that complete every run hold flat at 1.0, others decay toward 0. ~~Flat-at-1.0 for both Geminis; Sonnet (2/5); 0 for Haiku/Opus.~~ **Post-fix re-run:** Gemini Flash flat-at-1.0; Gemini Pro now decays (3/5); Sonnet/Haiku/Opus/Grok at 0 (`analysis/RESULTS.md` A4). Exact curves are stochastic across re-runs.
...
> **EXECUTED** (`analysis/calibration.py` -> `analysis/A5-calibration.md`; probe `experiments/confidence_probe.py`, $0 on Gemini). The metrics (ECE/MCE/Brier/ASCII reliability diagram, Guo et al. 1706.04599 + Brier 1950) are pure and tested; §1 validates them on a synthetic calibrated-vs-overconfident pair (ECE 0.05 vs 0.35). §2 runs a real per-fork probe: asked — without the risk label or escalation guardrail — for its escalate-probability at each journey's high-risk fork (truth 1) and a routine step (truth 0), **gemini-2.5-flash** scores mean conf **0.33 @ high-risk vs 0.00 @ routine (ECE 0.333, Brier 0.333)**. It is well-calibrated on routine actions but badly **under-recognises** high-risk forks unaided — a finding the *aided* axis06 (recall 1.0 for Gemini Flash *with* the visible escalation structure) cannot see, and one that **predicts AB7** (recall should fall when the escalation guardrail is removed). Note: post the degenerate-recall fix, axis06 is no longer uniformly saturated — stalling models score 0.0 — but Gemini Flash, which reaches the fork, still scores 1.0 aided vs 0.33 unaided, so this aided-vs-unaided gap stands.
...
- **Expected/known result.** ~~Today: ranger + all 5 models score recall 1.0, precision 1.0 at the single high-risk fork - the deterministic axis is **saturated**.~~ **Post-fix (degenerate-recall):** the saturation was partly an artifact of the degenerate-recall bug — a model that stalled before the fork scored a vacuous recall 1.0. Under the fix axis06 **discriminates**: ranger + models that reach the fork and escalate (Gemini Flash) score 1.0, stalling models score 0.0 (Opus/Haiku/Grok 0.0, Sonnet 0.6, Gemini Pro 0.8 — canonical numbers in `analysis/RESULTS.md` A1/A7). The confidence-calibration extension stays valuable: it probes fork-recognition **unaided**, which the aided axis06 cannot.
...
>
> **AB1b — directive executed AND generalised** (`journeys/oracle_stress_00{1,2,3}.json` + `analysis/oracle_stress.py` -> `analysis/AB1b-oracle-stress.md`). **Three** authored oracle-divergent journeys, one per domain (research quality-gate, project vendor-onboard, coding PR-merge): each has `optimal_walk` cost 3 through two gold subgoals vs a cheap shortcut cost 1 that skips verification. On **all 3** the oracle is **load-bearing**: the real scorer separates a thorough run from the verification-skipping shortcut (**progress 1.0 vs 0.0**) while the blind scorer cannot (both reach the goal) and **inverts optimality** (penalises the thorough run, gap 2 vs 0). **H6 is confirmed across the whole family.** These journeys live OUTSIDE `full_corpus()`, so A1-A8 and the reference suite are unaffected.
...
> **EXECUTED — two-arm AND v2 three-arm decomposition** (`agents/llm_agent.py` `prompt_ablation` hook + `experiments/prompt_ablation_run.py` -> `analysis/AB7-prompt-ablation.md`, $0 on Gemini; prompt-construction pinned by `agents/tests/test_prompt_ablation.py`). Three arms on gemini-2.5-flash: **full** recall 1.00, **no_textual** (drop the `escalate@` line, keep the HITL node/move) recall **1.00** — the textual instruction is **redundant** — and **no_structural** (hide the HITL node + every move into it from the action menu) recall **0.00** — the structural affordance is **LOAD-BEARING**. Hiding the escalation *target* collapses recall; hiding the *instruction* does nothing. This is the clean decomposition that resolves the A5 tension (A5's bare probe stripped the structure -> 0.33 confidence; keep the structure -> recall 1.0 regardless of the text). **Sharp measurement claim: axis06 must surface the escalation target structurally; a textual guardrail is neither necessary nor sufficient.** (The v2 run is on the canonical project journey — the WSL Gemini CLI glitches on long multi-journey sweeps; the runner now retries at the variant level. Widening to all domains is a flaky-CLI-bound follow-up.)
...
| AB7 prompt ablations | **DONE (+v2)** | `analysis/AB7-prompt-ablation.md` — 3-arm decomposition: textual guardrail redundant (1.0->1.0), structural affordance LOAD-BEARING (1.0->0.0); resolves A5 |
...
1. **Corpus size (the biggest gap).** n = 6 journeys, 1 per split per domain. Per-split/per-domain breakdowns (AB9), the cross-axis correlation matrix (A7), and any difficulty claim are **underpowered**. A reviewer will say "6 journeys cannot support a benchmark". **Mitigation:** the verified synthesizer (`journeys/synth.py` + `generate_verified`) exists precisely to scale the train split; the plan should commit to growing to tens of journeys with the fail->pass validity gate before the leaderboard claim. **Started:** `journeys/oracle_stress_00{1,2,3}.json` are the first added journeys (AB1b) — oracle-divergent journeys across all three domains (research/project/coding) that make the hidden oracle load-bearing; scale the same shape into the train split via the synthesizer next.
