...
1. **Corpus size (the biggest gap).** n = 6 journeys, 1 per split per domain. Per-split/per-domain breakdowns (AB9), the cross-axis correlation matrix (A7), and any difficulty claim are **underpowered**. A reviewer will say "6 journeys cannot support a benchmark". **Mitigation:** the verified synthesizer (`journeys/synth.py` + `generate_verified`) exists precisely to scale the train split; the plan should commit to growing to tens of journeys with the fail->pass validity gate before the leaderboard claim.

2. **Safety axes have no variance yet -> degenerate correlations.** axis07 ASR = 0 and axis10 = 1.0 for every model tested. The "foresight x safety are independent dimensions" claim (H2) cannot be *demonstrated* by a correlation when one of the two has zero variance. **Mitigation:** add journeys/prompts where safety genuinely varies (a fraction of models *do* leak / *do* propose unsafe edits) - otherwise H2 rests on construction, not data.

3. **axis10 discriminates the gate, not models.** Benign frontier models never proposed an unsafe self-edit even gate-OFF; the gate necessity is shown only by the synthetic `_RogueDreamer`. So axis10 is currently a property test of the harness, not a model-ranking axis. **Mitigation:** an adversarial-Dreamer prompt suite (poisoned objectives, jailbreak-style "improve by shortcutting") to see whether any real model proposes a forbidden-opening edit - only then does axis10 rank models. (DGM detector-gaming, arXiv 2505.22954, shows this *can* happen under the right pressure.)
